...
let if_error b str =
  if b then print_string str
  else ();;

let check_result data x =
  let all = check_all data x in
  let unique = check_unique x in
  let out = check data x in
  if_error (not all) "Brak elementow! ";
  if_error (not unique) "Powtorzenia! ";
  if_error (not out) "Zla kolejnosc! ";
  all && unique && out;;
...
let one_test length = 
  ...
  in
    ...
    | None -> if cyklicznosc = 1 then (print_string "cyklicznie " ; print_string "test ok")
      else print_string "nie oczekiwano wyjatku! test fail!"
    | Some(x) -> if cyklicznosc = 1 then print_string "oczekiwano wyjatku! test fail!"
      else if check_result data x then print_string "test ok"
