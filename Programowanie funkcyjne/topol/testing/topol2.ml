...
(*  [dfs ((map1, map2), wyn) stos glebokosc] przeszukuje w głąb graf
    ...
    "iteracji". [glebokosc] obsługuje przypadek powrotu do wierzchołka
    odwiedzonego - wyjątek rzucamy tylko przy kroku w głąb. Porównujemy
    ją z głębokością leżacą na [stos] razem z elementem.
...
let rec dfs ((graph, colors), wyn) stos prev_depth =
  match stos with
  | [] -> ((graph, colors), wyn)
  | (el, cur_depth)::tl ->
    if not (PMap.mem el colors) then
      let colors = PMap.add el 2 colors in
      dfs ((graph, colors), el::wyn) tl cur_depth
    else
      let col = PMap.find el colors in
      if col = 1 && (cur_depth >= prev_depth) then raise Cykliczne
      else if col = 1 && (cur_depth < prev_depth) then
        let colors = PMap.add el 2 colors in 
        dfs ((graph,colors), el::wyn) tl cur_depth
      else if col = 2 then dfs ((graph, colors), wyn) tl cur_depth
      else
        let colors = PMap.add el 1 colors in 
        let neighbours = (PMap.find el graph) in
        let neighbours = make_pairs neighbours (cur_depth+1) in
        let stos = neighbours @ stos in
        dfs ((graph, colors), wyn) stos cur_depth;;
