...
#define CHECK(var, err) if ((var) != CL_SUCCESS) { std::cout << "Error: " << err << std::endl; return 1; };
#define ALIGNED(x, align) (x + (x % align != 0 ? align - (x % align) : 0))


#ifndef KERNEL_NAME
  #define KERNEL_NAME "heap"
#endif

#ifndef LOCAL_THREADS
  #define LOCAL_THREADS 32;
#endif
...
cl_uint d; // Number of dimensions
cl_uint l; // Number of labels
cl_uint q; // Query size
cl_uint k; // Number of neighbors


cl_float* input_points; // Input points dimensions
cl_uint input_points_size;
cl_mem input_points_buffer;

cl_float* input_query; // Input query points
cl_uint input_query_size;
cl_mem input_query_buffer;

cl_int* output_query; // Output
cl_uint output_query_size;
cl_mem output_query_buffer;


const size_t local_threads = LOCAL_THREADS;
size_t global_threads = 0;



cl_ulong start_time_all;
cl_ulong end_time_all;
cl_ulong start_time_kernel;
cl_ulong end_time_kernel;
