...
#define RAND_SCOPE 10
#define MAX_TRIES 10
#define EPS       1.0e-9
#define DELTA     1.0e-9
#define MAXIT     30
#define DER_PREC  1.0e+8
...
char lepsze(FLOAT a[], FLOAT b[]){
  FLOAT f1[2], fD1[4], f2[2], fD2[4];
  wielomian_funkcja(2, NULL, a, f1, fD1);
  wielomian_funkcja(2, NULL, b, f2, fD2);
  return (f1[0] * f1[0] + f1[1] * f1[1]) < (f2[0] * f2[0] + f2[1] * f2[1]) ? 1 : 0;
}
...
int faktoryzuj_wielomian(void){
  ...
  while(wielomian_wymiar > 3){
    ...
    div[1] = 1.0;
    div[2] = 1.0;
    ret = SysNewton(2, wielomian_funkcja, NULL, EPS, DELTA, MAXIT, div + 1);
    anything = ret != 0 ? 1 : 0; /* mamy jakies przyblizenie */
    memcpy(best, div, sizeof(FLOAT) * 3);

    /* losowaniesz punktow startowych */
    if(ret != 1){
    ...
    memcpy(div, best, sizeof(FLOAT) * 3);
